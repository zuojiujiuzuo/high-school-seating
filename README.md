# 班阵

面向高中教师的本地排座工作台。提供名单管理、教室布局、关系规则、座位方案生成和文件导出的一体化工作流。

线上版本：<http://139.196.115.78:8088/>

## 本地运行

```bash
nvm use 24.14.0
npm install
npm run dev
```

浏览器访问 `http://127.0.0.1:1420`。进入页面会先显示免责声明；确认后直接进入精简的排座工作区，顶部不提供模式切换。

首次进入工作台或新建班级时，画布区域会显示毛玻璃布局选择层；确认桌型、行列和模板后生成该班级的初始座位。
当前桌型、行列和护法座组合可以命名保存为本机自定义模板，之后新建班级或调整布局时可直接复用。

## 验证

```bash
npm run test:run
npm run build
```

Tauri 桌面壳已完成配置，但本机需要先安装 Rust 工具链和对应平台依赖后才能运行 `npm run tauri dev`。

桌面版已接入 Ed25519 离线授权验证，首次导入后不访问服务器；正式安装包需使用 Apple/Windows 代码签名证书构建。签发与发布说明见 [docs/OFFLINE_LICENSE_AND_SIGNING.md](docs/OFFLINE_LICENSE_AND_SIGNING.md)。

## 当前可操作内容

- 在 Canvas 中点击或框选学生；滚轮缩放，移动工具拖动画布。
- 将左侧待入座名单拖到空座位。
- 拖动已入座学生到另一座位，观看双向弧线换位动效。
- 画布工具悬浮在底部，可选择、移动画布、调整讲台、清空或禁用座位。
- 每个画布工具都会显示即时操作提示，按 Esc 可随时回到选择工具。
- 在同一个“排座”页面创建四类多人规则、查看冲突诊断并生成单个方案。
- 有规则的学生在座位左侧显示深色“规”标记；点击可查看该学生参与的全部规则。
- 切换六种布局预设并立即更新 Canvas；布局页隐藏名单，可调整教室门位置，并用“禁用”工具切换带粗叉标识的禁用座位。
- 使用撤销/重做、纯净视图和降低动态效果。
- 在顶部切换班级和座位版本；每个组合拥有独立的本地座位快照。
- 从待入座名单上传任意列结构的 `.xlsx` 或 `.csv`；文件至少包含两列，并在预览页确认必填的姓名、学号，可继续映射性别、班级、成绩等级（A/B/C/D）、身高和标签，直接修改单元格或删除整行。多个标签可用顿号、逗号或分号分隔；缺少性别时仍可导入，但防早恋模式会提示补充信息并暂不可用。
- 根据算法与权重生成一个方案；不满意可再次生成，预览后仍可直接拖动学生微调。
- 班级确认初始座位并首次导入学生后，默认勾选“防早恋”和“身高模式”自动生成一次方案；性别缺失时仅启用身高模式并提示补充。
- 导出 Excel、PNG 或 PDF 文件到当前设备；PNG 与 PDF 均提供保留画布的普通版和纯白重排的精简版。
- 使用右上角主题切换器在“简约”和“猫爪”高保真主题之间即时切换；选择会保存在本机。
- 点击右上角醒目的“免责声明”，查看本地数据、AI 合规、人工复核、维护及责任说明。

正式产品需求见 [docs/PRODUCT_REQUIREMENTS.md](docs/PRODUCT_REQUIREMENTS.md)，免责声明草案见 [docs/LEGAL_NOTICE.md](docs/LEGAL_NOTICE.md)，视觉令牌见 [docs/DESIGN_SYSTEM.md](docs/DESIGN_SYSTEM.md)。

## 服务器部署

仓库跟踪已构建的 `dist/`，服务器只需拉取仓库并让 Nginx 指向该目录：

```bash
cd /srv/high-school-seating
git pull --ff-only
nginx -t && systemctl reload nginx
```

Nginx 示例配置见 [deploy/nginx/high-school-seating.conf](deploy/nginx/high-school-seating.conf)。修改源代码后，请先在 Node.js 20.19 以上环境执行 `npm run build` 并提交新的 `dist/`。
